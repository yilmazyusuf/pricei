    ...
    laravel.ajax.init = function () {
        ...
        $(document).on('submit', 'form.ajax', function (event) {
            ...
            $.ajax({
                ...
                beforeSend: laravel.ajax.beforeSendHandler,
        ...
        $(document).on('submit', 'form.ajax_upload', function (event) {
            event.preventDefault();
    ...
    laravel.ajax.beforeSendHandler = function (xhr,object) {
        ...
        laravel.ajax.errorHandler = function (event) {
            ...
            if (status == 422) {  //validation Error
                ...
                laravel.alert("Bu işlem için yetkiniz bulunmuyor.")
            } else if (status == 404) { //not found
                laravel.alert('Aradığınız sayfa bulunamadı.')
            } else if (status == 200) { //OK but not really :)
                laravel.alert('İşlem sonucu başarısız (Geçersiz JSON formatı)');
            } else if (status == 419) { //Session timeout :)
                laravel.alert('Uzun süreli aktif olmayan oturum, lütfen tekrar giriş yapın');
                location.reload();
            } else if (status == 0) {
                //Aborted request
            } else {
    ...
    laravel.errors.renderValidation = function (errors, form) {
        ...
        for (var fieldName in errors.errors) {
            ...
            if (error.hasOwnProperty(fieldName)) {
                var fieldErrors = error[fieldName];
